...
impl IncSolver {
    ...
    pub fn solve(&mut self) {
        ...
        for block in &self.blocks {
            for &var_idx in &block.variables {
                self.variables[var_idx].final_position = block.position + self.variables[var_idx].offset;
            }
        }
    ...
    fn merge_blocks(&mut self, left_block_id: usize, right_block_id: usize, constraint_id: usize) {
        ...
        let left_var_pos = self.blocks[left_block_id].position + self.variables[left_var_idx].offset;
        let right_var_pos = self.blocks[right_block_id].position + self.variables[right_var_idx].offset;
        ...
        for &var_idx in &right_vars {
            ...
            let old_abs_pos = self.blocks[right_block_id].position + self.variables[var_idx].offset + right_block_shift;
